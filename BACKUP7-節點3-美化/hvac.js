...
const HVACModule = {
    ...
    calculateRow: function(key, area, height, people) {
        try {
            ...
            let strEA;
            if (criteria.reqEA) {
                // [修改] 改為綠色 (text-success) 和打勾圖標
                strEA = `<span class="text-success" style="font-weight:700; font-size:1.1em;">
                            <i class="fas fa-check-circle"></i> 是 / YES
                         </span>`;
            } else {
    ...
    addItem: async function() { // 記得加 async
        ...
        if (!typeEl.value || !areaEl.value) {
            ...
            if (typeof RenoApp !== 'undefined') {
                await RenoApp.showCustomModal("資料不全", "請完整輸入類型與面積 (Type & Area required)", false);
            } else {
                alert("請完整輸入類型與面積"); // 後備方案
            }
            return;
        }
        // ... 後續邏輯不變
