...
const RenoApp = {
    ...
    init: function() {
        ...
        // ... (以下保持不變：讀取 localStorage, Device Selector, Payment Check 等) ...
        const savedUser = localStorage.getItem('reno_user');
        if (savedUser) {
            this.state.currentUser = JSON.parse(savedUser);
            
            // ✅ [修改] 加上 await 確保資料下載完才顯示畫面
            // 注意：因為 init 本身不能直接暫停整個網頁，我們用 .then 處理
        ...
        try {
            ...
            if (actionType === 'REGISTER') {
                ...
                if (updateRes.error) {
                    ...
                    await this.showCustomModal("註冊成功但Email有誤", "帳號已建立，但 Email 綁定失敗，請稍後在設定中重試。", false);
                } else {
                    await this.showCustomModal(
            ...
            if (error) {
                ...
                // 成功登入
                this.state.currentUser = { phone: phone, id: data.user.id };
