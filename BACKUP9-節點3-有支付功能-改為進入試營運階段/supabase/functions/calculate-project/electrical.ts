...
function getSplitLimit(type: string): number {
  // ✅ [修改] 1. 一般插座 -> 10個一組
  if (type.includes('General Socket') || type.includes('一般插座')) return CONSTANTS.SPLIT_LIMITS.GENERAL;
  
  // ✅ [修改] 2. 廚房插座 -> 2個一組
  if (type.includes('Kitchen Socket') || type.includes('廚房插座')) return CONSTANTS.SPLIT_LIMITS.KITCHEN;
